...
public class ShadowSampleViewManager extends GVRScript {
...
//		GVRScene scene = mGVRContext.getNextMainScene();
		...
		gvrDirectionalLight.setBias(0.001f);
								    //0.001f
		scene.setDirectionalLight(gvrDirectionalLight); /// Add Shadow Map light
														/// // TODO method
...
//		gvrDirectionalLight.setShadowGradientCenter(2);
		...
		case 0:
			...
			float ly = 7;
			float lz = 0; ////
			gvrDirectionalLight.setLightPosition((float) (lightDistance * x) + lx, ly, (float) (lightDistance * z) + lz);
...
//			gvrDirectionalLight.setShadowSmoothDistance(3f);
		...
		case 1:
			...
			gvrDirectionalLight.setLightDirection(0, 0, 0);
			gvrDirectionalLight.setSpotangle((float) Math.abs(countTime / 2f % 200 - 100) * 1.2f + 1);
			break;
		...
		case 2:
			...
			gvrDirectionalLight.setLightDirection(0, 0, 0);
			gvrDirectionalLight.setSpotangle(100);
			gvrDirectionalLight.setShadowSmoothSize((float) Math.abs((countTime / 100f) % 3));
			break;
		}
	}
	...
	private GVRSceneObject meshWithTexture(String mesh, String texture) {
		GVRSceneObject object = null;
		try {
			object = new GVRSceneObject(mGVRContext, new GVRAndroidResource(mGVRContext, mesh),
	...
	private GVRModelSceneObject addDinoAnim(GVRScene scene, float scale, float x, float y, float z) throws IOException {
//		GVRSceneObject object = meshWithTexture("TRex_NoGround.fbx", "t_rex_texture_diffuse.png");
